function main() {
    ...
    this.dom = {
        ...
        'data': document.getElementById('outerUI'),
...
main.prototype.init = function (mode, callback) {
    // 注册外置canvas
    main.canvas.outerUI = document.getElementById("outerUI").getContext('2d');
...
main.dom.body.onkeyup = function(e) {
    try {
        if (main.dom.startPanel.style.display == 'block' && document.getElementById('startImageBackgroundDiv').style.display == 'none' &&
...
main.dom.data.onmousedown = function (e) {
    try {
        ...
        if (loc == null) return;
        if (loc.outer) core.ui.statusBar.onclick(loc.x, loc.y);
        else main.core.ondown(loc);
    } catch (ee) { main.log(ee); }
}
